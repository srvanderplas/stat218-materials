...
true_false_qs <- tribble(
  ...
  "A boxplot is a visual display of the five-number summary", T, 6,
  "The IQR is the difference between the minimum and the maximum value", F, 6,
  "A two sample t-test requires that both groups are symmetric or that both groups have 20 observations and the sample distributions are not strongly skewed", T, 6,
  "$\\mu_d$ is the population average of the differences", T, 7,
  "$\\overline{x}_d$ is the average of the individual differences in the sample", T, 7,
  "Two samples are dependent if the data recorded on one sample are unrelated to those recorded on the other sample", F, 7,
  "A t-test for paired data is the same as a t-test for one sample continuous data", T, 7,
  "A matched pairs experiment is an experiment where the same units are measured multiple times, so that each unit is its own control", F, 7
)
...
\vspace{-2em}\hfill{\underline{\hspace{1in}}\large /32}
...
\vspace{-2em}\hfill{\underline{\hspace{1in}}\large /20}
...
```{r fig.width = 8, fig.height = 4, out.width = "100%", message = F, warning = F}
pup <- tibble(
...
1. [1pt] What is the population of interest?    
\longanswer\answer{The company's current canine customers. (Also acceptable - all canines)}

2. [2pt] Which group has the largest median? Draw a vertical line corresponding to the approximate median on the chart for each group.    
...
3. [2pt] Suppose we would like to compare the average itchiness of the pups receiving the original treatment to the average itchiness of pups receiving the new treatment (Original - New). What is the parameter of interest, in words and in symbols?    
...
4. [2pt] Set up the appropriate null and alternative hypotheses to examine the parameter of interest.    
\longanswer\answer{$H_0: \mu_o - \mu_n = 0$\\$H_A: \mu_o - \mu_n \neq 0$}

5. [2pt] Are theory-based validity conditions met for this problem?    
...
6. [3pt] Suppose that the new group of pups scratches on average `r mean(filter(pup, trt == "New")$scratch)` times per day (SD = `r sd(filter(pup, trt == "New")$scratch)`) and the old group of pups scratches on average `r mean(filter(pup, trt != "New")$scratch)` times per day (SD = `r sd(filter(pup, trt != "New")$scratch)`). What is the observed statistic?    
...
7. [2pt] What is the standard deviation of the difference (i.e. the standard deviation of the statistic?)    
\longanswer\answer{
...
8. [3pt] What is the test statistic, $t$?    
...
9. [3pt] What is your conclusion in the context of the study?    
...
\vspace{-2em}\hfill{\underline{\hspace{1in}}\large /12}
The veterinary medicine company wants to estimate the reduction in scratching after taking the new medication. It recruits 60 dogs; for the first 30 days, the dogs' scratches are counted but no treatment is given (Control). Then, each dog takes the new medication for 30 days, and at the end of that 30 days, the company counts each dog's scratches for another 30 days (Med).
...
pup <- tibble(
  id = rep(1:60, times = 2),
  trt = rep(c("Control", "Med"), each = 60),
  scratch = rnorm(120, mean = rep(c(30, 20), each = 60), sd = 5)
)

pup_wide <- pup %>% pivot_wider(names_from = trt, values_from = scratch) %>%
  mutate(diff = Control - Med)

ggplot(data = pup, aes(x = trt, y = scratch, group = id)) + geom_line() + 
  xlab("Test Period") + ylab("Average # Scratches per day")
```

1. [3pt] What is the difference between this type of study design and the previous study design? What is the advantage to this type of design over the previous study?    
\longanswer\answer{This is a paired design (repeated measures). Each dog serves as its own control, which means that we can more precisely estimate the size of the effect because we reduce the variability between groups.}

2. [4pt] The goal of this experiment is to estimate the reduction in scratching after taking the new medication. What do you need to do in order to meet this experiment's goal? Describe the steps in the analysis process.    
\longanswer[6em]\answer{We need to calculate a confidence interval for $\mu_d$, the population average reduction in scratching after taking the new medication. To do this, we need to calculate the reduction in scratching for each dog, and then compute the average and std. deviation of the differences (Control - Med) to determine the overall reduction. We then use techniques from Chapter 3 to construct a 2SD interval for the population mean difference.}

3. [3pt] Suppose the sample average difference is `r round(mean(pup_wide$diff), 2)` and the sample standard deviation is `r round(sd(pup_wide$diff), 2)`. Can you calculate a 2SD interval for the population average difference?    
\longanswer[6em]\answer{First, we compute the standard error: $s/\sqrt{n} = `r round(sd(pup_wide$diff), 2)`/\sqrt{60} = `r round(sd(pup_wide$diff)/sqrt(60), 2)`$. The interval is then $`r round(mean(pup_wide$diff), 2)` \pm 2 \times `r round(sd(pup_wide$diff)/sqrt(60), 2)` = `r sprintf("(%0.2f, %0.2f)", round(mean(pup_wide$diff), 2) -2 * round(sd(pup_wide$diff)/sqrt(60), 2), round(mean(pup_wide$diff), 2) +2 * round(sd(pup_wide$diff)/sqrt(60), 2))`$.}

4. [2pt] Interpret your interval from 3 in the context of the problem.    
\longanswer[6em]\answer{We are 95\% confident that the flea and tick treatment reduces the population average number of dog scratches by between `r round(mean(pup_wide$diff), 2) -2 * round(sd(pup_wide$diff)/sqrt(60), 2)` and `r round(mean(pup_wide$diff), 2) +2 * round(sd(pup_wide$diff)/sqrt(60), 2)` per day.}
