...
- January 16: [start](#3) - [end](#15)

- January 21: [start](#16) - [end](#41)

- January 23: [start](#42) - [end](#69)

- January 28: [start](#70) - [end](#99)

- January 30: [Review](#100)
]
.pull-right[
.large[By Section]
...
1. Research question: .red[Are novice players are less likely to <br/>choose scissors?]
...
- Is $\hat p_{scissors}$ less than the probability specified in the <br/>null hypothesis?<br/>    
...
.bottom[.grey[Note that the <strong>p-value</strong> is not the same p as the sample proportion, <math xmlns="http://www.w3.org/1998/Math/MathML"><mstyle color="gray">
  <mrow class="MJX-TeXAtom-ORD">
    <mover>
      <mi>p</mi>
      <mo stretchy="false">&#x005E;<!-- ^ --></mo>
...
set.seed(2016100700)
df <- tibble(x = rbinom(1000, 12, 1/3)/12)

ggplot(df, aes(x = x)) + geom_bar() + 
  scale_x_continuous("Proportion of Scissors in 12 trials", breaks = (0:12)/12,
                     labels = sprintf("%d/12", 0:12), limits = c(0, 1)) +
...
ggplot(df, aes(x = x)) + geom_bar() + 
  ...
  annotate("segment", y = 0, yend = Inf, x = 2/12, xend = 2/12, color = "red")
...
ggplot(df, aes(x = x)) + geom_bar() + 
  ...
  annotate("segment", y = 0, yend = Inf, x = 2/12, xend = 2/12, color = "red") +
  annotate("text", y = 175, x = 1/12, label = "atop(displaystyle(atop(In~the,direction)),of~H[A])", color = "red", parse = T, vjust = 0) + 
  annotate("segment", x = 1.75/12, xend = 0, y = 150, yend = 150, color = "red", arrow = arrow(ends = "last", length = unit(0.1, "inches")) )
```
...
df <- df %>%
  mutate(extreme = factor(x <=2/12, labels = c("Not extreme", "Extreme"))) 
...
$$\underbrace{P(\hat p \leq 2/12)}_\text{under random chance} \approx P(\underbrace{p^\ast \leq 2/12}_\text{simulation}) =  \frac{193}{1000} = .193$$
...
$$P(\hat p \leq 1/12) \approx P(p^\ast \leq 1/12) = \frac{58}{1000} = 0.058$$
...
one_sided <- sprintf("$p=%.2f$", tbl %>% filter(type == "One Sided") %>% `[[`("extreme") %>% mean(na.rm = T))
two_sided <- sprintf("$p=%.2f$", tbl %>% filter(type == "Two Sided") %>% `[[`("extreme") %>% mean(na.rm = T))
```
- There are 100 dots. To find the p-value, we count the dots which are as or more extreme than what we observed (0.8)

- For a 1-sided test of whether we observed more heads (0.8, 0.9, and 1.0), `r one_sided`

- For a 2-sided test of whether the coin is fair (0.8, 0.9, 1.0, and 0.2, 0.1, 0.0), `r two_sided`
...
\begin{align}H_0: \pi &= 0.05\\\\ H_A: \pi &> 0.05\end{align}
...
- Calculate the standardized statistic, $z$
\begin{align}
z &= \frac{\hat p - \pi}{\sigma} \\\\
& = \frac{\frac{23}{250} - 0.05}{0.01378} \\\\
& = \frac{0.092 - 0.05}{0.01378} \\\\
& = \frac{0.042}{0.01378} \approx 3.05
\end{align}
...
- Conduct a test of your hypothesis using theory-based methods. Use the theory-based inference app to get a p-value for your test.    
<img alt="Theory-based inference app screenshot" src="figure/Review-Ch1_theory-one-prop.png" width="80%"/>
...
<img alt="Theory-based inference app screenshot" src="figure/Ch1Review-Simulation-Screenshot.png" width="80%"/>
...
$$z = \frac{\hat p - \pi_0}{\sigma} = \frac{.1167 - .05}{.0199} = 3.35$$
  ...
  - p-value: (From the app) $$p \approx 0.0020$$
...
You conduct a second sample of 120 adults, and this time, you find that 14 individuals have dyslexia.
  ...
  - Using the histogram below, draw lines indicating the cutoff(s) for extreme values, draw one or more arrows corresponding to the direction of $H_A$, and shade in the values considered "extreme".
```{r dyslexia-hist-ans, fig.width = 8, fig.height = 4}
set.seed(20934702)
x <- rbinom(1000, 120, .05)
extreme <- factor(x >= 14, labels = c("Not extreme", "Extreme"))
qplot(x = x, fill = extreme, geom = "histogram", binwidth = 1, color = I("black")) + theme_bw() + 
  scale_x_continuous("Simulated Number of Individuals Testing Positive for Dyslexia") + 
  scale_fill_manual("", values = c("grey80", "red")) + 
  annotate("segment", x = 13.5, xend = 13.5, y = 0, yend = 150, color = "red") + 
  annotate("text", x = 13.5, y = 155, label = "Observed Value", hjust = 0.5, color = "red") + 
  annotate("segment", x = 13.75, y = 100, xend = 16, yend = 100, color = "red", arrow = arrow()) + 
  annotate("text", x = 16.5, y = 100, label = "H[A]", color = "red", parse = T) + 
  theme(legend.title = element_blank(), legend.position = c(.01, .99), legend.justification = c(.01, .99), legend.background = element_rect(fill = "transparent"))
```
Note: This is a different simulated dataset than the one from the app. 

.small[.bottom[(This is how I would test whether you understand how simulation works)]]
...
.small[.bottom[The important parts are in bold]]
...
.center[
<img src="figure/Edison.png" alt = "A photo of Edison the Jack Russell" width = "75%"/>    
...
.center[
<iframe width="560" height="315" src="https://www.youtube.com/embed/zVeoQTOTIuQ" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
]
